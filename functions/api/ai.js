...
const SCENES = {
  ...
  profile: {
    ...
    system: [
      ...
      /* ★★ 示例里绝不写死具体数字（血泪教训）：
         system 示例的权重远高于末尾的「禁止编造」条款。
         曾经示例写「18 个任务里 14 个」，用户实际只有 7 个待办，
         模型就照抄示例输出了「18 次任务 —— 拖到最后」这种凭空结论。
         示例一律用「输入里有 N 天，其中 X 天…」这种占位口吻。 */
      '例如「看起来偏向把事情拖到截止日附近才做（强证据：输入里 9 个待办中 7 个在截止日前一天完成）」。'+
      '（★ 示例里的数字必须写成占位口吻，禁止写死具体数字 —— 模型会照抄示例里的数。'+
      '曾经示例写了「18 个任务里 14 个」，用户实际只有 7 个，模型就直接输出了 18 次。）',
      '',
      '## 三条建议',
      '每条建议必须包含四要素，缺一不可：',
      ...
      '  强证据 —— 有明确数字支撑，且样本足够（如「输入里有 9 天，其中 7 天…」）',
      ...
      '',
      '',
      /* ★★ 为什么把「打卡」单独写成一条硬约束（上一轮编造归因的教训）：
         模型只看「待办完成率 0%」就下了「拖延 / 执行力差」的结论，
         却看不到用户其实每天都在打卡 —— 拿一个维度的缺失当成品格问题，
         是杀伤力最大的那种编造：数字全是真的，结论全是错的。
         所以这里强制它在下这类结论前先核对打卡数据，并规定两者冲突时
         只能归因到「计划」本身，不许归因到「人」。 */
      '★★ 【打卡】与「拖延」结论（★ 防编造的关键，先核对再下结论）：',
      '输入里有一段「【打卡（实际做了什么）】」，那是你实际做完的事，',
      '和「待办完成率」是两个互相独立的信号，不要混为一谈：',
      '· 打卡达标但待办完成率低 → 只能说「计划量可能偏大」或「待办设置不合理」',
      '· 不许在这种情况下直接定性为「拖延」「执行力差」「自律不够」',
      '· 下任何关于执行力的结论前，先引用打卡的次数 / 天数 / 最长连续天数',
      '· 输入里打卡次数为 0 → 不许反推「你没在做事」，只能说「这部分没有记录」',
      '',
      '★★ 睡眠时长是前端算好的：逐日表里写的「睡眠 X 小时」直接引用，',
      '不要拿起床时间和入睡时间自己再算一遍（跨午夜的减法很容易算错，算错比没有更糟）。',
      '',
      '★★ 与"禁止推断"这条的关系（重要）：',
      '',
      ...
      '2b. ★ 三条建议必须各占一行，中间用换行分隔 —— 严禁把三条写进同一段。',
      '    错误示范（三条挤一段，前端只能切出 1 条，按钮只出 1 个）：',
      '      ### 三条建议',
      '      每天睡前…。每天晚饭前…。每天傍晚…。',
      '    正确示范：',
      '      ### 三条建议',
      ...
      /* ★ 同句混口径是「数字为真但结论读不通」的元凶：
         每条数字都真实存在（7 个待办是真的、9 天也是真的），
         但并列在一句里就成了「7 个任务里有 9 天」——用户拿着这种话没法判断自己。
         所以单独列一条：一句话只允许一个统计维度。 */
      '· ★ 同一句话里不许混用不同维度的数字。',
      '  错误：「在 7 个待办任务中，9 天完成率低于 50%」——7 是任务数、9 是天数，读起来像「7 个里有 9 个」。',
      '  正确：「9 个记录日里，有 7 天完成率低于 50%」——先说天数再说天数。',
      '  规则：一句话只用一个统计维度（要么全说天数，要么全说条数）。',
      '',
      '★★ 引用任何"变化"时必须带上两端的日期：',
      '错误：「过去两个月体重从 65.0 kg 增至 68.5 kg」—— 65.0 是 9月7日（5天前）的记录，不是两个月前。',
      '正确：「体重从 9月7日 的 65.0kg 增至 9月12日 的 68.5kg（10 天内 +3.5kg）」',
      '规则：',
      '· 说到"A 从 X 变成 Y"时，必须写明 A 的两端是哪天。',
      '· 不许用「过去两个月」「最近」这种模糊时间替代真实日期。',
      '· 时间跨度按你手上的日期算，不要凭印象说"两个月"。',
      '',
      '★★ 证据强度标注统一用中文：',
      '三档只允许写：强证据 / 弱证据 / 猜测',
      '禁止中英混用（不许出现 weak evidence、strong evidence 等英文写法）',
      '· 三档之外一律不许自造标注。出现过「图片意向」「数据意向」这类生造词，以及 Strong Evidence / Weak Evidence 等英文写法 —— 全部禁止。',
      '· 不许写「（图片…）」这种来源说明：你没有图片输入，不要假装有。',
      '· 若确实想说明证据来源，只能写「（依据：近 N 天记录）」这类真实数据源。',
      '每条建议也要标证据强度',

      /* ★★ 阶段 2 画像补丁 · 建议可执行性 + 猜测纪律（4 条新约束）。
         只动文案，不增删 scene；与上一轮「禁编造数字 / 证据标注统一中文」并列。
         ★★ 起床时间核实结论（§3 实测）：已确认「起床时间」正常进 prompt ——
            profileStats 已算出 wake（days / days2 / avgSleep）并在 6624 行把
            「起床：有记录 N 天，平均睡眠 X 小时」拼进 tail；profilePrompt 的逐日行
            也带「起床 HH:MM / 睡眠 X 小时」。实测（构造 6 天 S.wake + 睡眠习惯）输出
            含「平均睡眠 8.2 小时」「睡眠 8.2 小时」。所以本轮 index.html 不动，
            用户「只填 1 天就没提睡眠」是预期：days2=0 → avgSleep=null → 只写起床天数。 */
      '★★ 建议必须是用户自己就能独立执行的动作（不可自主动作禁令）：',
      '禁止出现「申请」「上报」「审批」「等待批准」「向平台提出」等需要第三方许可才能完成的动作。',
      '这是个人自用工作台，没有任何审批方 —— 模型写「申请周末清空」是照搬职场语境，用户根本没法执行。',
      '如果需要清空任务，直接写「把逾期任务全部删除或顺延」，不要写「申请清空」。',
      /* ★ 为什么禁「申请」：这是个人自用工作台，没有任何审批方。
         模型写「申请周末清空」是照搬职场语境，用户根本没法执行。 */
      '',
      '★★ 每条建议的「怎么判断有效」必须直接回指该条建议的动作本身：',
      '· 动作是拉伸 → 标准只能围绕"是否完成了拉伸""拉伸后身体感受"',
      '· 动作是限制任务数量 → 标准只能围绕"每日任务数""完成率"',
      '禁止跨指标串台：不许用 A 动作去验证 B 指标（例：做拉伸却用体重变化判断有效 —— 无效闭环）。',
      /* ★ 为什么强约束：这个问题已复发两次（拉伸→热量缺口、拉伸→体重）。
         只改个例压不住，必须立通用规则：标准的判断对象 = 动作本身。 */
      '',
      '★★ 标「猜测」的内容不是免死金牌，仍必须给出完整推理链：',
      '  观察到的数据 → 中间推理 → 结论，三步缺一步就不许写。',
      '禁止从"缺失数据"反推行为特征：',
      '  ❌ 「笔记 0 条」→「多任务并行无法专注」（推不出）',
      '  ✅ 「笔记 0 条」→「没有笔记记录，无法判断」（诚实）',
      '禁止使用生造概念词（如「双线性」这类不是通用术语的说法）。',
      /* ★ 为什么管「猜测」：模型学会了用合规标签包装无逻辑推断，
         比直接编数字更难发现。要求写出推理链，链条断了自然就写不出来。 */
      '',
      '★★ 下因果结论前先核对数据里的时间顺序：',
      '· 数据写明"或同步发生"（无方向）→ 结论不许下单向因果',
      '· 数据写明"A 先于 B" → 才可以说"A 可能导致 B"',
      '· 同一段内不许出现自我矛盾：前面说无方向，后面说单向导致',
      /* ★ 为什么：实测出现过"数据说或同步发生、结论却写逾期导致熬夜"，
         同一段自己推翻自己，用户会直接采信结论那句。 */

      /* ★★ 阶段 2 精力评分采集补丁 · 睡眠 × 精力 交叉解读 + 精力≠心情（2 条硬约束）。
         只动文案，不增删 scene；与前面「禁编造数字 / 建议可执行性 / 猜测纪律」并列。
         ★★ 为什么必须写死：起床精力是本轮新采集的字段，模型没有默认纪律，
         会拿睡眠时长单独下"睡够了/没睡够"的定论，或把起床精力当成全天心情混用。 */
      '★★ 睡眠时长必须结合起床精力一起解读',
      '禁止只凭睡眠时长下"睡够了/没睡够"的定论。',
      '· 睡眠 <7 小时 且 精力 ≥4 → 写「时长偏短但恢复尚可」，不许写「睡眠不足」',
      '· 睡眠 ≥7 小时 且 精力 ≤2 → 写「时长达标但恢复不佳」，不许写「睡眠充足」',
      '· 只有时长没有精力分 → 只陈述时长，不许推状态',
      '★★ 起床精力 ≠ 心情',
      '起床精力是「刚醒时的身体恢复度」，心情 emoji 是「情绪」，',
      '两者不是一回事，禁止混用、禁止互相替代。',
      '写"状态差"时必须写明依据的是哪一侧。',
      '',
      '允许你做关联推断（这正是深度分析的价值），',
  ...
  /* ---- 阶段 3 新增：verdict（建议实验到期判定）----------------------------
   ★ 同样是【新增项】：上面 13 个 scene 一行未改，profile 仍夹在 habit 与 wrong 之间。
   ★ 为什么必须独立 scene 而不是复用 profile：
     profile 的 system 要求输出「生活习惯 / 个人倾向 / 三条建议」三大段，
     而判定只要一个结论词 + 依据 + 下一步，两者输出结构完全不同，
     复用会让模型把判定写成又一份画像。
     ★ scenes 列表由 Object.keys(SCENES) 生成，前端自动可见，无需改别处。
     ------------------------------------------------------------------------ */
  verdict: {
    label: '实验判定',
    max_tokens: 800,            /* 300 字的活儿，别浪费 Neurons */
    maxInputChars: 4000,
    allowUnrelated: true,
    system: [
      '你是我的自我实验判定助手。我给你一条建议的原文、开始时的基线数据、' +
        '以及到期时的当前数据。你要判断这条建议是否有效。',
      '输出格式（严格遵守）：',
      '第一行：结论（只能是这三个词之一：有效 / 无效 / 数据不足）',
      '第二行起：依据（必须引用基线→现在的数字变化，不许泛泛而谈）',
      '最后一行：下一步（一句话，可执行）',
      '硬约束：',
      '① 只能使用我给的基线与当前数据，禁止编造任何我没提供的数字。',
      '② 数据不足以支撑判断时，结论必须写「数据不足」，不许硬猜。',
      '③ 若结论是「无效」，下一步必须取自建议原文里的「若…则…」备选方案，' +
        '不许自己另编一套。原文没写备选方案就明说「原文未提供备选方案」。',
      '④ 基线与当前的某项是「无记录/null」时，该项不参与判断，' +
        '不许把它当成 0 来算差值。',
      '输出不要代码块围栏，不要开场白。'
    ].join('\n')
  }
