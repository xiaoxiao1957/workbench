...
const SCENES = {
  ...
  review: {
    ...
    system: [
      ...
      '输出四段，每段一个小标题：',
      '① 做了什么 —— 只列记录里真实出现的事，按主题归类，不要按日期流水账。',
      '② 状态如何 —— 结合心情记录和打卡数据，说这段时间整体状态是向上、持平还是下滑。',
      '③ 一个观察 —— 挑出一件我自己可能没意识到的事（比如某个反复出现的模式、某件悄悄在改变的事）。只给一个，宁缺毋滥。',
      '④ 下一段的一条建议 —— 只要一条，具体到"什么时候做什么"，不要给清单。',
      '硬约束：',
      '- 禁止编造记录里没有的事，禁止把没做的说成做了。',
      '- 记录太少（少于 3 条）时，直接说明"这段时间记录不多，回顾会比较粗略"，然后基于仅有的内容写，不要硬凑篇幅。',
      '- 不要安慰，不要说"你已经很棒了"这类空话。做得不好就直说。',
...
const FRAMEWORKS = [
  ...
  },
  {
    ...
    system: '用认知行为疗法（CBT）的框架帮我梳理这些记录里的想法。' +
      '结构：① 识别出 2-3 个"自动思维"（原样引用记录里的话）；' +
      '② 指出每个思维里的认知扭曲（如非黑即白、灾难化、读心术、以偏概全），说明为什么；' +
      '③ 给每个自动思维一个更平衡、更符合证据的替代想法；' +
      '④ 给一个可以做的小行动。' +
...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await gateway({
      ...
      /* 批次二：新增的三个可选字段。老请求不带它们（= undefined），
