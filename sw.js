...
/* ① 缓存版本号：v3 → v4 → v5
      v3：把 HTML 主文档从「缓存优先」改为「网络优先」，必须升版，
          否则已安装的旧 SW 不会生效、老缓存也不会被清掉。
      v4：模块六改了 manifest.webmanifest 的 theme_color / background_color。
          主文档已走网络优先，但 manifest 走的是缓存优先，
          不升版的话老用户拿到的还是旧色值的 manifest（PWA 启动画面颜色对不上）。
      v5：云端同步改为走同源代理 /api/webdav/*。老版本对同源 GET 是「缓存优先」，
          备份文件一旦被塞进运行时缓存，「从云端拉取」永远拿到旧数据 ——
          升版清掉全部旧缓存，并新增 /api/ 直接放行规则。 */
...
self.addEventListener('fetch', event => {
  ...
  // 1.5) 同源 /api/ 开头的请求直接放行（v5 新增）
  //      这是云端同步的同源代理（functions/api/webdav），备份文件必须永远走网络：
  //      老版本对同源 GET 缓存优先，备份文件会被塞进运行时缓存，
  //      之后「从云端拉取」永远拿到第一次的旧数据 —— 这是绝对不能发生的。
  try {
    if (new URL(req.url).pathname.startsWith('/api/')) {
      return;
    }
  } catch (e) { /* URL 解析失败按原逻辑继续 */ }
